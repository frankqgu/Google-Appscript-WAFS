...
function getStudents() { 
  ...
  var getLastRow = studentSheet.getLastRow();  
  return studentSheet.getRange(2, 1, getLastRow - 1, 1).getValues();  
}
...
function TotalHours()
{
  ...
  for(var i = 0; i < totals.length; i++)
  {
    ...
    mainSheet.getRange(2+i,9).setValue(totals[i][1]).setFontSize(12);  
  } 
